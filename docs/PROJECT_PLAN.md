...
볼거리의 진입 기준은 **광역급 랜드마크만 허용하는 것이 아니라, 그 동네에서 일부러 들를 이유가 있는 지역 대표 목적지까지 포함**하는 것입니다.

- Tier A: 서울/수도권 차원의 대표 랜드마크·대형 문화/공원/관광 목적지 → 포함
- Tier B: 해당 동네를 대표하며 친구에게 “이 역 갔으면 한번 가봐”라고 추천할 만한 시장·거리·문화공간·산책지·공원 → 포함
- Tier C: 일반적인 동네 놀이터·소공원·아파트 주변 쉼터·특색이 약한 근린시설 → 제외
- A/B 후보가 없으면 0개를 허용하며, 숫자를 채우기 위해 억지로 추천하지 않음

즉 품질 기준은 “유명세 절대값”보다 **지역 대표성 + 실제 방문가치**입니다.
